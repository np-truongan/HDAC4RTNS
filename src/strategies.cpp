...
//
//  Stores data[0] verbatim; each subsequent byte stores the
//  signed difference to its predecessor (wrapping arithmetic
//  in uint8 space, which is lossless and reversible).
// ============================================================
...
//
//  Packs two 4-bit nibbles into one byte.
//  PRECONDITION: every value in `data` must be in [0, 15].
//  Calling code (engine.cpp) is responsible for only routing
//  data here when that invariant holds.
//
//  Encoded layout: high nibble = data[2i], low nibble = data[2i+1].
//  If data.size() is odd, the last byte's low nibble is 0.
// ============================================================
...
Chunk compressZSTD(const Chunk& data) {
    ...
    size_t compressedSize = ZSTD_compress(
        out.data(), maxSize,
        data.data(), data.size(),
        1);   // level 1: fast, still better ratio than LZ4
...
// ============================================================
//  Gzip (via zlib's compress2 with gzip-compatible wrapper)
//
//  We use deflateInit2 with windowBits=15|16 to get true gzip
//  framing — important for comparability with external tools.
// ============================================================
Chunk compressGZIP(const Chunk& data) {
    z_stream zs{};
    if (deflateInit2(&zs, Z_BEST_SPEED, Z_DEFLATED,
                     15 | 16,   // 15 = max window, 16 = gzip header
